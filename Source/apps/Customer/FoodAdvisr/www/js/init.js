...
function bindAll(){
  ...
  //    $('.login-logo').show();
    ...
    body.on('click','.targ-intol-item',function(){
      setTimeout(function(){appdata.saveIntolerancePrefs()},50);
      return false;
    });
    ...
    body.on('click','.act-login-settings',function(){
      ...
      op+='<br/>';
        ...
        t.next().addClass('hide');
				//$(this).next().find('.toggle').parent().hide().removeClass('notactive');
				//$(this).next().find('.toggle.active').parent().show();
			}else{
				t.addClass('expanded');
        t.next().removeClass('hide');
				//$(this).next().find('.toggle').parent().show();
			}
		});
